...
import PrepositionsItemCard from "./prepositions-item-card";
import { DeviceType, deviceType } from "expo-device";

function PrepositionsScreen() {
  ...
  const redirectTo = useRef<{ slug: string; prepositionsName: string }>({
    slug: "",
    prepositionsName: "",
  });
  const [prepositionsItems, setPrepositionsItems] =
    useState<{ id: string; prepositions: string }[]>(undefined);

  useEffect(() => {
    LocalStorage.getItemDefault(PREPOSITIONS_LIST_KEY).then((val) => {
      let ruffStoredContents = val ? JSON.parse(val) : [];
      const storedContents = ruffStoredContents;
      setPrepositionsItems(storedContents);
    });
  }, []);
  ...
  const redirectToNextScreenAfterAdmobInterstitial = () => {
    if (redirectTo.current.slug || redirectTo.current.prepositionsName) {
      router.push(
        `./prepositions-detail?selectedPrepositionsSlug=${redirectTo.current.slug}&&selectedPrepositionsName=${redirectTo.current.prepositionsName}`
      );
    }
  };
  ...
  const renderItem = useCallback(
    (item) => {
      return (
        <PrepositionsItemCard
          key={item?.item?.id ?? ""}
          item={item?.item ?? { id: "", prepositions: "" }}
          onPress={(slug: string, prepositionsName: string) => {
            redirectTo.current = { slug, prepositionsName };
    ...
    [canShowAdmobInteratitial, isLoaded]
  );
  ...
  const renderEmptyState = useCallback(() => {
    return <EmptyStateNormal title="Prepositions not found!" />;
  }, []);
...
export default PrepositionsScreen;
